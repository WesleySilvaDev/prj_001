<?php
   ...
   if($url_request == "todas"){
       ...
       $consultaAcompanhanteUser = $pdo->query("SELECT * FROM users WHERE $url_request = 'SIM' AND  conta = 'ATIVA' ORDER BY RAND() ");
   }
   ...
   <body data-rsssl=1 class="page-template page-template-categorias page-template-categorias-php page page-id-71701 _masterslider _ms_version_3.7.10 menu-position-top menu-style-v2 no-sidebar not-wrapped-widgets no-ads-on-smartphones no-ads-on-tablets with-infinite-scroll page-fixed-width with-transparent-menu">
      ...
      <article id="page-71701" class="pluto-page-box post-71701 page type-page status-publish hentry">
         ...
         <div class="tgrakz" style="margin:10px;">
            <h5>       <?php 
                       if($url_request == 'massagem_tantrica'){  $url_request= ('MASSAGEM TANTRICA');  }
                       if($url_request == 'suggar_baby'){  $url_request= ('SUGGAR BABY');   } 
                       if($url_request == 'desp_solteiro'){  $url_request= ('DESPEDIDA DE SOLTEIRO');  } 
                       echo strtoupper($url_request);
                       ?>
                  ...
                  <div class="figure-link-w">
                     <a href="../perfil/<?=$userConsulta['username']?>"  class="figure-link ">
                        <figure class="abs-image" >
                           <img src="<?=$imagem;?>" class="attachment-pluto-index-width size-pluto-index-width wp-post-image" alt=""  style=" object-fit: cover; height: 220px; border-radius: 5px 5px 0px 0px;"/>
      ...
      <div style="float:left">© Copyright 2021 - Nosso Love </div>
      </div>
      </div>
      </div>
      </div>
